...
```
The resulting tidy dataset contains a large number of observations spread across all available months and ZIP codes in New York City. In total, it includes `r n_obs` rows, covering `r n_zip` unique ZIP codes and representing `r n_neighborhood` distinct neighborhoods. This structure ensures that each row corresponds to a unique combination of ZIP code and date, with associated information on rental prices and location.
